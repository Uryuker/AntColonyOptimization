...
public class RandomGraph {
	...
	public void generateGraph(int nodes, float connexity){
		...
		for(float iedge=1;iedge<=maxEdges;iedge++){
			...
			while(noDupplicate){
				...
				if(graph.getNode(String.valueOf(rstart)).getEachEdge()!=null){
					for(Edge e :graph.getNode(String.valueOf(rstart)).getEachEdge()){
						if(e!=null){
							if(e.getOpposite(graph.getNode(String.valueOf(rstart)))==graph.getNode(String.valueOf(rend))){
								noDupplicate=true;	
							}else if(Integer.parseInt(e.getOpposite(graph.getNode(String.valueOf(rstart))).getId())==rstart && Integer.parseInt(e.getSourceNode().getId())==rend){
									noDupplicate=true;	
							}
						}
					}
			...
			graph.addEdge(String.valueOf(iedge),graph.getNode(String.valueOf(rstart)).getIndex(),graph.getNode(String.valueOf(rend)).getIndex());
			...
			{
				//Test to see if all nodes are connected otherwise we connect it with the next node
				for(Node n: graph.getEachNode()) {
					if(n.getEachEnteringEdge()==null && n.getEachLeavingEdge()==null){
						...
						if(Integer.parseInt(n.getId())==nodes){
							...
							graph.addEdge(String.valueOf(iedge), Integer.parseInt(n.getId()), Integer.parseInt(n.getId())+1);
							graph.getEdge(String.valueOf(iedge)).addAttribute("length", rlength);
							graph.getEdge(String.valueOf(iedge)).addAttribute("pheromones", 0);
							iedge++;
						}
